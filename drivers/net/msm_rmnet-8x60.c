...
)
{
  ...
  while (msg_len > 0)
  {
    ...
    *p++ = '\n';
    *p = '\0';
    pr_info(MODULE_NAME "%s: %s", direction, output_buf);
  }
}
#endif
...
static void smd_net_data_handler(unsigned long arg)
{
	...
	for (;;) {
		...
		if (RMNET_IS_MODE_IP(opmode) ? (sz > dev->mtu) :
						(sz > (dev->mtu + ETH_HLEN))) {
			pr_err(MODULE_NAME "[%s] rmnet_recv() discarding packet len %d (%d mtu)\n",
			...
			if (skb == NULL) {
				pr_err(MODULE_NAME "[%s] rmnet_recv() cannot allocate skb\n",
				       dev->name);
			} else {
				skb->dev = dev;
