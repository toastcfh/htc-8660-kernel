...
static struct usb_function *rmnet_sdio_function;
...
static void rmnet_notify_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	case -ESHUTDOWN:
		...
		break;
	default:
		RMNET_ERR("%s: rmnet notifyep error %d\n", __func__, status);
		/* FALLTHROUGH */
	case 0:
		...
		if (status) {
			...
			RMNET_ERR("%s: rmnet notify ep enq error %d\n", __func__, status);
		}
		break;
	}
}
...
static void rmnet_set_modem_ctl_bits_work(struct work_struct *w)
{
	...
	RMNET_DEBUG("%s: cbits_to_modem:%d\n",
...
static void rmnet_open_sdio_work(struct work_struct *w)
{
	...
	retry_cnt++;
	RMNET_DEBUG("%s: usb rmnet sdio open retry_cnt:%d\n",
			__func__, retry_cnt);
...
static int rmnet_set_enabled(const char *val, struct kernel_param *kp)
{
	int enabled = simple_strtol(val, NULL, 0);
	RMNET_INFO("%s: %d\n", __func__, enabled);


	android_enable_function(rmnet_sdio_function, enabled);
	return 0;
}

static int rmnet_get_enabled(char *buffer, struct kernel_param *kp)
{
	buffer[0] = '0' + !rmnet_sdio_function->hidden;
	RMNET_INFO("%s: %d\n", __func__, buffer[0] - '0');
	return 1;
}
module_param_call(enabled, rmnet_set_enabled, rmnet_get_enabled, NULL, 0664);
...
#endif
	rmnet_sdio_function = &dev->function;
