...
void register_for_radio(struct uart_port *uport);
#endif
...
#ifdef CONFIG_LOCOSTO_UART2DM_HANDSHAKE
	unsigned char pda_int_bb; /* high: BB can sleep */
...
static int msm_hs_init_clk_locked(struct uart_port *uport)
{
	...
	pr_debug("%s set GPIO_PDA_STATUS low\n",__func__);
#endif
	...
	pr_debug("%s clk_state = MSM_HS_CLK_ON\n",__func__);
	return 0;
}
...
#ifdef CONFIG_LOCOSTO_UART2DM_HANDSHAKE
	{
		pr_debug("%s ignore linux PM, use msm_hs_request_clock API\n",__func__);
					...
					struct msm_dmov_errdata *err)
{
	...
	pr_debug("%s msm_hs_read UARTDM_SR_ADDR(0x%lx)=0x%lx\n", __func__, (unsigned long)uport->membase+UARTDM_SR_ADDR, status);
	...
	pr_debug("%s msm_hs_read UARTDM_RX_TOTAL_SNAP_ADDR(0x%lx)=0x%x\n", __func__, (unsigned long)uport->membase+UARTDM_RX_TOTAL_SNAP_ADDR, rx_count);
	/*
...
static int msm_hs_check_clock_off_locked(struct uart_port *uport)
{
	...
	if (msm_uport->clk_state != MSM_HS_CLK_REQUEST_OFF){
		pr_debug("%s ret=-1(clk_state != REQUEST_OFF) -\n", __func__);
		return -1;
	}
	if (!uart_circ_empty(tx_buf)){
		pr_debug("%s ret=-1(!uart_circ_empty) -\n", __func__);
		return -1;
	}
	if (msm_uport->tx.dma_in_flight){
		pr_debug("%s ret=-1(tx.dma_in_flight) -\n", __func__);
		return -1;
	}
	if (msm_uport->imr_reg & UARTDM_ISR_TXLEV_BMSK){
		pr_debug("%s ret=-1(ISR_TXLEV) -\n", __func__);
		return -1;
	}
	if (!(msm_uport->imr_reg & UARTDM_ISR_RXLEV_BMSK)) {
		pr_debug("%s ret=-1(!ISR_RXLEV) -\n", __func__);
		return -1;
	}
	...
	if (!(sr_status & UARTDM_SR_TXEMT_BMSK)){
		pr_debug("%s ret=0(!SR_TXEMT) -\n", __func__);
		return 0;  /* retry */
	}
#ifdef CONFIG_LOCOSTO_UART2DM_HANDSHAKE
	/* PDA has nothing send to BB, so set PDA_INT_BB to high */
	if (msm_uport->pda_int_bb == 0) { /*PDA_INT_BB low: BB cannot sleep */
		gpio_configure(OBOEA_GPIO_GSM_PDA_INT_BB, GPIOF_DRIVE_OUTPUT | GPIOF_OUTPUT_HIGH);
		pr_debug("%s set GPIO_PDA_INT_BB high-\n",__func__);
		msm_uport->pda_int_bb = 1; /* PDA_INT_BB high : BB can sleep */
	}
	...
	pr_debug("%s msm_uport->clk_req_off_state=%d\n", __func__, msm_uport->clk_req_off_state);
	...
	case CLK_REQ_OFF_START:
		 ...
		 */
		dsb();
		pr_debug("%s ret=0(CLK_REQ_OFF_START) -\n", __func__);
		return 0;  /* RXSTALE flush not complete - retry */
	case CLK_REQ_OFF_RXSTALE_ISSUED:
		pr_debug("%s ret=0(CLK_REQ_OFF_RXSTALE_ISSUED) -\n", __func__);
		return 0;  /* RXSTALE flush not complete - retry */
	case CLK_REQ_OFF_FLUSH_ISSUED:
		pr_debug("%s ret=0(CLK_REQ_OFF_FLUSH_ISSUED) -\n", __func__);
		return 0;  /* RXSTALE flush not complete - retry */
	...
	if (msm_uport->rx.flush != FLUSH_SHUTDOWN) {
		...
		pr_debug("%s ret=0(!FLUSH_SHUTDOWN) -\n", __func__);
	...
	if (radio_state){
		...
		pr_debug("%s set GPIO_PDA_STATUS high\n",__func__);
	}
	else{
		pr_debug("%s NOT set GPIO_PDA_STATUS high due to radio_off\n",__func__);
	}
#endif
	msm_uport->clk_state = MSM_HS_CLK_OFF;
	pr_debug("%s clk_state = MSM_HS_CLK_OFF\n",__func__);
	wake_unlock(&msm_uport->dma_wake_lock);
	if (use_low_power_rx_wakeup(msm_uport)) {
		msm_uport->rx_wakeup.ignore = 1;
#ifdef CONFIG_LOCOSTO_UART2DM_HANDSHAKE
		pr_debug("%s NOT enable_irq rx_wakeup.irq(%d)\n",__func__, msm_uport->rx_wakeup.irq);
#else
		enable_irq(msm_uport->rx_wakeup.irq);
#endif
	}
	pr_debug("%s ret=1(SUCCESS) -\n", __func__);
	return 1;
}
...
static irqreturn_t msm_hs_isr(int irq, void *dev)
{
	...
	pr_debug("%s msm_hs_read UARTDM_MISR_ADDR(0x%lx)=0x%lx\n", __func__, (unsigned long)uport->membase+UARTDM_MISR_ADDR, isr_status);
...
void msm_hs_uart2_request_clock_off_locked(struct uart_port *uport)
{
	...
	if (msm_uport->clk_state == MSM_HS_CLK_ON) {
		...
		pr_debug("%s clk_state = MSM_HS_CLK_REQUEST_OFF\n",__func__);
...
#endif
#ifdef CONFIG_LOCOSTO_UART2DM_HANDSHAKE
		pr_debug("%s NOT disable_irq rx_wakeup.irq(%d)\n",__func__, msm_uport->rx_wakeup.irq);
#else
...
#ifdef CONFIG_LOCOSTO_UART2DM_HANDSHAKE
	struct msm_hs_port *msm_uport = UARTDM_TO_MSM(uport);
	msm_uport->pda_int_bb = 0; /*PDA_INT_BB low: BB cannot sleep */
#endif
...
static irqreturn_t msm_hs_rx_wakeup_isr(int irq, void *dev)
{
	...
	if (msm_uport->bb_int_pda == 1) { /*BB_INT_PDA high: PDA cannot sleep */
		pr_debug("set IRQF_TRIGGER_HIGH\n");
...
static int msm_hs_startup(struct uart_port *uport)
{
	...
	pr_debug("%s : UART2DM_NS_REG=0x%x  +\n", __func__, ns_reg);
	...
	pr_debug("irq(msm_hs_uart_locosto)=%d\n", uport->irq);
	...
	if (use_low_power_rx_wakeup(msm_uport)) {
		...
		pr_debug("irq(msm_hs_rx_wakeup_locosto)=%d\n", msm_uport->rx_wakeup.irq);
...
static int msm_hs_probe(struct platform_device *pdev)
{
	...
	/* for debug */
	pr_debug("Locosto\n");
	pr_debug("pdev->id=%d, pdev->name=%s\n", pdev->id, pdev->name);
	...
	pr_debug("%s mapbase=0x%lx membase=0x%lx irq=%d\n", __func__,
	...
	*/
	...
	pr_debug("dma_tx_channel=%d\n", msm_uport->dma_tx_channel);
	pr_debug("dma_rx_channel=%d\n", msm_uport->dma_rx_channel);
	...
	pr_debug( "dma_tx_crci=%d\n", msm_uport->dma_tx_crci);
	pr_debug("dma_rx_crci=%d\n", msm_uport->dma_rx_crci);
	...
	msm_hs_set_bps_locked(uport, 921600);
	pr_debug( "set bps to 921600 by default\n");

	data = msm_hs_read(uport, UARTDM_MR1_ADDR);
	data |= UARTDM_MR1_CTS_CTL_BMSK;
	data |= UARTDM_MR1_RX_RDY_CTL_BMSK;
	msm_hs_write(uport, UARTDM_MR1_ADDR, data);
	pr_debug( "enable CTSRTS by default\n");
	...
	msm_uport->clk_off_delay = ktime_set(0, 10000000);  /* 10ms */
...
static int __init msm_serial_hs_init(void)
{
	...
	if (get_kernel_flag() & BIT21){
		uart2_debug_mask = 1;
		printk(KERN_DEBUG MODULE_NAME " %s enable uart2 debug msg\n", __func__);
	}
	else{
		uart2_debug_mask = 0;
		printk(KERN_DEBUG MODULE_NAME " %s disble uart2 debug msg\n", __func__);
	}
