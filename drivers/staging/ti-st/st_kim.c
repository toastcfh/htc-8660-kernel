 ...
 */
void kim_int_recv(struct kim_data_s *kim_gdata, const unsigned char *data,
				  long count)
{
	...
	while (count) {
		if (kim_gdata->rx_count) {
			...
			case ST_W4_HEADER:
				plen =
					(unsigned char *)&kim_gdata->rx_skb->
					data[1];
		...
		ptr++;
		count--;
		kim_gdata->rx_skb = alloc_skb(1024 + 8, GFP_ATOMIC);
		if (!kim_gdata->rx_skb) {
...
{	&ldisc_install.attr, &uart_dev_name.attr, &uart_baud_rate.attr,
	&uart_flow_cntrl.attr, NULL,
};

static struct attribute_group uim_attr_grp = { .attrs = uim_attrs,
};
...
static const struct file_operations version_debugfs_fops = {
	/* version info */
	.open = kim_version_open, .read = seq_read, .llseek =
	seq_lseek, .release = single_release,
};

static const struct file_operations list_debugfs_fops = {
	/* protocols info */
	.open = kim_list_open, .read = seq_read, .llseek =
	seq_lseek, .release = single_release,
};
...
static int kim_probe(struct platform_device *pdev)
{
	...
	debugfs_create_file("version", S_IRUGO, kim_debugfs_dir, kim_gdata,
						&version_debugfs_fops);
	debugfs_create_file("protocols", S_IRUGO, kim_debugfs_dir, kim_gdata,
						&list_debugfs_fops);
...
static struct platform_driver kim_platform_driver = { .probe =
	kim_probe, .remove = kim_remove, .suspend =
		kim_suspend, .resume = kim_resume, .driver = {.name =
			"kim", .owner =
				THIS_MODULE,
		},
};
