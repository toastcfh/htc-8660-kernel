...
static void tvout_msm_hpd_state_timer(unsigned long data)
{
	unsigned long flags;
	if (tvout_msm_state->bootup)
		tvout_msm_state->bootup = false;

#ifdef CONFIG_SUSPEND
	spin_lock_irqsave(&tvout_spin_lock, flags);
	if (tvout_msm_state->pm_suspended) {
		spin_unlock_irqrestore(&tvout_spin_lock, flags);
	...
	*/
	...
	//tvout_check_status();
	schedule_work(&tvout_msm_state->hpd_state);
}
...
static void tvout_msm_hpd_state(struct work_struct *work)
{
	tvout_check_status();
}
...
static int tvout_off(struct platform_device *pdev)
{
	...
	if (tvout_msm_state->cable_state == 0)
...
#ifdef CONFIG_HAS_EARLYSUSPEND
static void tvout_early_suspend(struct early_suspend *h)
{
	if(tvout_msm_state->disp_powered_up == TRUE) {
		tvout_msm_turn_on(FALSE);
	}
}

static void tvout_late_resume(struct early_suspend *h)
{
	if(tvout_msm_state->disp_powered_up == TRUE) {
		tvout_msm_turn_on(TRUE);
	}
...
static int __devinit tvout_probe(struct platform_device *pdev)
{
	...
	spin_lock_init(&tvout_spin_lock);
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	tvout_msm_state->early_suspend.suspend = tvout_early_suspend;
	tvout_msm_state->early_suspend.resume = tvout_late_resume;

	register_early_suspend(&tvout_msm_state->early_suspend);
#endif
	...
	if (fb_dev) {
		...
		if (tvenc_pdata->poll) {
			...
			tvout_msm_state->bootup = true;
			mod_timer(&tvout_msm_state->hpd_state_timer, jiffies
				+ msecs_to_jiffies(15000));
			/* Cable is not plugged so turn off clock */
			tvenc_set_encoder_clock(CLOCK_OFF);
		}
	} else
...
static int tvout_device_pm_suspend(struct device *dev)
{
	unsigned long flags;
	spin_lock_irqsave(&tvout_spin_lock, flags);
	pr_info("%s cable_status=%d\n", __func__, tvout_msm_state->cable_state);
	if (tvout_msm_state->pm_suspended) {
		spin_unlock_irqrestore(&tvout_spin_lock, flags);
		return 0;
	}
	if (tvenc_pdata->poll) {
		...
		disable_irq(tvout_msm_state->irq);
		if (tvout_msm_state->cable_state == 1)
			tvenc_set_encoder_clock(CLOCK_OFF);
	}
...
void tvout_enable_detection(boolean on)
{
	//uint32 reg;
	unsigned long flags;
	pr_info("%s on/off=%d\n", __func__, on);
	...
	if (tvout_msm_state->pm_suspended) {
		if (on)
			tvout_msm_state->cable_state = 3;
		else
			tvout_msm_state->cable_state = 4;
	} else {
		if (on) {
			tvenc_set_encoder_clock(CLOCK_ON);
			tvout_msm_state->cable_state = 1;
			enable_irq(tvout_msm_state->irq);
		} else {
			tvout_msm_state->cable_state = 0;
		}
	}
	pr_info("%s:cable_state=%d\n", __func__, tvout_msm_state->cable_state);
	spin_unlock_irqrestore(&tvout_spin_lock, flags);
}
...
static int __init tvout_init(void)
{
	...
	INIT_WORK(&tvout_msm_state->hpd_state, tvout_msm_hpd_state);
	return 0;
