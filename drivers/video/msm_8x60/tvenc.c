...
int tvenc_set_clock(boolean clock_on)
{
	int ret = 0;
	if (clock_on) {
		/* we must set clock rate before enabl it */
		/*if (tvenc_pdata->poll) {*/
		if (1) {
		...
		/*fix teenc clk not off during suspend */
		if (1)/*(tvenc_pdata->poll)*/
...
#ifdef CONFIG_MSM_BUS_SCALING
		if (tvenc_bus_scale_handle > 0)
			msm_bus_scale_client_update_request(tvenc_bus_scale_handle, 1);
#else
		if (mfd->ebi1_clk)
			clk_enable(mfd->ebi1_clk);
#endif
