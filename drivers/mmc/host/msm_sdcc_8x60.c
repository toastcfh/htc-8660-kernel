...
msmsdcc_dma_exec_func(struct msm_dmov_cmd *cmd)
{
	...
	writel_relaxed((unsigned int)host->curr.xfer_size, host->base + MMCIDATALENGTH);
...
msmsdcc_dma_complete_tlet(unsigned long data)
{
	...
	if (host->mmc->card && host->mmc->card->type == MMC_TYPE_SD) {
		...
		if (sd_dma_tick >= 3000)
			printk("SD_DMA tasklet time >= %d ms\n", sd_dma_tick);
	}
				...
				struct mmc_command *cmd, u32 *c)
{
	...
	*c |= (cmd->opcode | MCI_CPSM_ENABLE);
	...
	else if (is_svlte_platform(host->plat))
		mod_timer(&host->req_tout_timer,
...
msmsdcc_pio_irq(int irq, void *dev_id)
{
	...
	if (status & MCI_RXACTIVE && host->curr.xfer_remain < MCI_FIFOSIZE) {
		...
		writel_relaxed((readl_relaxed(host->base + MMCIMASK0) & (~(MCI_IRQ_PIO))) | 0,
				host->base + MMCIMASK0);
		dsb();
	}
		...
		if ((host->plat->dummy52_required) &&
			...
			if (status & (MCI_PROGDONE | MCI_CMDCRCFAIL |
				...
				pr_info("%s: datalength 0x%x\n", mmc_hostname(host->mmc),
					readl_relaxed(host->base + MMCIDATALENGTH));
				pr_info("%s: cmd 0x%x\n", mmc_hostname(host->mmc),
					readl_relaxed(host->base + MMCICOMMAND));
				pr_info("%s: arg 0x%x\n", mmc_hostname(host->mmc),
					readl_relaxed(host->base + MMCIARGUMENT));
			}
			break;
		}
...
msmsdcc_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	spin_lock_irqsave(&host->lock, flags);
	clks_on = host->clks_on;
	if (!clks_on) {
		/* force the clocks to be on */
		msmsdcc_setup_clocks(host, true);
		/*
		 * give at least 2 MCLK cycles delay for clocks
		 * and SDCC core to stabilize
		 */
		msmsdcc_delay(host);
	}
...
static void msmsdcc_enable_sdio_irq(struct mmc_host *mmc, int enable)
{
	...
	if (enable) {
		...
		writel_relaxed(readl_relaxed(host->base + MMCIMASK0) | MCI_SDIOINTOPERMASK,
			       host->base + MMCIMASK0);
		spin_unlock_irqrestore(&host->lock, flags);
	} else {
		host->mci_irqenable &= ~MCI_SDIOINTOPERMASK;
...
static int sdio_al_resume(struct msmsdcc_host *host)
{
	unsigned long time_to_wait;
	if (!host->plat->get_mdm2ap_status)
		return 0;
	if (host->plat->get_mdm2ap_status() == 1)
		return 0;
	...
	if (host->plat->get_mdm2ap_status() == 0)
		pr_info("%s: mdm is not out of vddmin\n",
			mmc_hostname(host->mmc));
...
#ifdef CONFIG_MSM_SDIO_AL
	struct msmsdcc_host *host = mmc_priv(mmc);
	if (host->plat->is_sdio_al_client && host->sdio_awakening) {
		sdio_al_resume(host);
		host->sdio_awakening = 0;
	}
#endif
#if 0
