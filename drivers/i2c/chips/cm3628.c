...
struct cm3628_info {
	...
	int (*re_power)(uint8_t); /* re_power to the chip */
	uint32_t als_kadc;
	uint32_t als_gadc;
	uint16_t golden_adc;
...
static void sensor_irq_do_work(struct work_struct *work)
{
	...
	if (add == lpi->ALS_slave_address ) {
		...
		pr_err("[PS][CM3628 error]%s error: \n"
		"unkown interrupt: 0x%x!, fail_count %d",
		__func__, add, fail_count);
		fail_count++;
	}
	if (lpi->re_power != NULL && fail_count > 20) {
		fail_count = 0;
		initial_cm3628(lpi);
		lpi->re_power(1);
		initial_cm3628(lpi);
