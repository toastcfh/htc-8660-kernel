...
static void pm8058_l11_on(int en)
{
	static struct regulator *vreg_l11;
	int rc;
	vreg_l11 = regulator_get(NULL, "8058_l11");
	if (IS_ERR(vreg_l11)) {
		pr_aud_err("%s: Unable to get 8058_l11\n", __func__);
		return ;
	}
	if (en) {
		rc = regulator_set_voltage(vreg_l11, 2850000, 2850000);
		if (rc)
			pr_aud_err("%s: unable to set voltage to 2850000",
				__func__);

		rc = regulator_enable(vreg_l11);
		if (rc)
			pr_aud_err("%s: Enable regulator 8058_l11 failed\n", __func__);

		regulator_put(vreg_l11);
	} else {
		rc = regulator_disable(vreg_l11);
		if (rc)
			pr_aud_err("%s: disable regulator 8058_l11 failed\n", __func__);
...
void rider_snddev_emic_pamp_on(int en)
{
	...
	if (en) {
		...
		gpio_set_value(PM8058_GPIO_PM_TO_SYS(RIDER_AUD_MIC_SEL), 1);
	}
}
