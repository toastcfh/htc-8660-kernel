...
#ifdef HTC_USE_SPIN
/* There is a condition that the mutex will be used within the spinlock when calling smd_close().
 * That will cause a BUG_ON(). So we use spinlock instead of mutex to avoid it.
 */
static DEFINE_SPINLOCK(smd_creation_spin);
#else
...
#ifdef HTC_USE_SPIN
	spin_lock_irqsave(&smd_creation_spin, flags);
	list_add(&ch->ch_list, &smd_ch_closed_list);
	spin_unlock_irqrestore(&smd_creation_spin, flags);
#else
