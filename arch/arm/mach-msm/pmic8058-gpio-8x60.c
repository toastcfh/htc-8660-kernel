...
static int sleep_read_gpio_bank(int gpio, int bank, u8 *data)
{
	int rc;
...
int pmic8058_dump_gpios(struct seq_file *m, int curr_len, char *gpio_buffer)
{
	...
	for (gpio = 0; gpio < PM8058_GPIOS; gpio++) {
		memset(read_buf, 0, sizeof(read_buf));
		len = 0;

		for (i = 0; i < PM8058_GPIO_BANKS; i++) {
			rc = sleep_read_gpio_bank(gpio, i, &bank[i]);
			if (rc)
				pr_err("pmic failed to read bank %d\n", i);
		}

		len += sprintf(read_buf + len, "GPIO[%2d]: ", gpio);

		val = pm8058_gpio_get(&pm8058_gpio_chip, gpio);

		/* print the mode and the value */
		mode = (bank[1] & PM8058_GPIO_MODE_MASK) >>
				PM8058_GPIO_MODE_SHIFT;
...
EXPORT_SYMBOL(pmic8058_dump_gpios);
