...
#define DEBUG_SYNC_DATA_COUNT	5
struct sync_debug_entry {
	unsigned long long timestamp;
	struct msm_clock *dst_clk;
	uint32_t src_clk_val;
	uint32_t dst_clk_val;
	uint32_t sleep_offset;
	uint32_t non_sleep_offset;
	uint32_t exit_sleep;
};

struct sync_debug_data {
	struct sync_debug_entry entry[DEBUG_SYNC_DATA_COUNT];
	int index;
};

static struct sync_debug_data timer_sync_debug_data;

#define DEBUG_TICK_DATA_COUNT 10
struct tick_debug_data {
	unsigned long long timestamp[DEBUG_TICK_DATA_COUNT];
	int index;
};

static DEFINE_PER_CPU(struct tick_debug_data, msm_timer_tick_debug);

struct next_event_debug_data_entry {
	uint32_t alarm;
	uint32_t now;
...
struct next_event_debug_data {
	struct next_event_debug_data_entry entry[DEBUG_TICK_DATA_COUNT];
	int index;
};

static DEFINE_PER_CPU(struct next_event_debug_data, msm_timer_next_event_debug);

void msm_timer_print_sync_debug(void) {
	int index = timer_sync_debug_data.index;
	printk("Dumping timer sync debug data\n");
	do {
		struct sync_debug_entry entry = timer_sync_debug_data.entry[index];
		if (entry.dst_clk) {
			unsigned long nanosec_rem;
			nanosec_rem = do_div(entry.timestamp, 1000000000);

			printk(KERN_INFO "[%5lu.%06lu] %s: "
				"src %u, dst %u, new offset %u + %u, exit_sleep %d\n",
				(unsigned long) entry.timestamp, nanosec_rem / 1000,
				entry.dst_clk->clocksource.name, entry.src_clk_val,
				entry.dst_clk_val, entry.sleep_offset, entry.non_sleep_offset,
				entry.exit_sleep);
		}
		index = (index + 1) % DEBUG_SYNC_DATA_COUNT;
	}
	while(index != timer_sync_debug_data.index);
}

static void save_sync_debug(struct msm_clock *dst_clk,
	uint32_t src_clk_val, uint32_t dst_clk_val, uint32_t sleep_offset,
	uint32_t non_sleep_offset, int exit_sleep) {

	int index = timer_sync_debug_data.index;
	struct sync_debug_entry *entry = &timer_sync_debug_data.entry[index];

	entry->timestamp = cpu_clock(smp_processor_id());
	entry->dst_clk = dst_clk;
	entry->src_clk_val = src_clk_val;
	entry->dst_clk_val = dst_clk_val;
	entry->sleep_offset = sleep_offset;
	entry->non_sleep_offset = non_sleep_offset;
	entry->exit_sleep = exit_sleep;

	timer_sync_debug_data.index = (index + 1) % DEBUG_SYNC_DATA_COUNT;
}

void msm_timer_print_tick_debug(void) {
	struct tick_debug_data *data = &__get_cpu_var(msm_timer_tick_debug);
	int index = data->index;

	printk("Dumping last %d timer tick timestamp\n", DEBUG_TICK_DATA_COUNT);
	do {
		unsigned long long timestamp = data->timestamp[index];
		unsigned long nanosec_rem;
		nanosec_rem = do_div(timestamp, 1000000000);
		printk(KERN_INFO "[%5lu.%06lu]\n", 	(unsigned long)timestamp, nanosec_rem / 1000);
		index = (index + 1) % DEBUG_TICK_DATA_COUNT;
	}
	while(index != data->index);
}

void msm_timer_print_next_event_debug(void) {
	struct next_event_debug_data *data = &__get_cpu_var(msm_timer_next_event_debug);
	int index = data->index;

	printk("Dumping last %d set next event data\n", DEBUG_TICK_DATA_COUNT);
	do {
		struct next_event_debug_data_entry *entry = &data->entry[index];
		unsigned long long timestamp = entry->timestamp;
		unsigned long nanosec_rem;
		nanosec_rem = do_div(timestamp, 1000000000);
		printk(KERN_INFO "[%5lu.%06lu] now = %u, alarm = %u,"
			" timer count = %u, match = %u, enable = %u\n",
			(unsigned long)timestamp, nanosec_rem / 1000, entry->now, entry->alarm,
			entry->timer_count, entry->timer_match, entry->timer_enable);
		index = (index + 1) % DEBUG_TICK_DATA_COUNT;
	}
	while(index != data->index);
}

static void save_tick_debug(void) {
	struct tick_debug_data *data = &__get_cpu_var(msm_timer_tick_debug);
	int index = data->index;
	data->timestamp[index] = cpu_clock(smp_processor_id());
	data->index = (index + 1) % DEBUG_TICK_DATA_COUNT;
}

static void save_next_event_debug(uint32_t alarm, uint32_t now, uint32_t timer_count,
	uint32_t timer_match, uint32_t timer_enable) {
	struct next_event_debug_data *data = &__get_cpu_var(msm_timer_next_event_debug);
	int index = data->index;
	struct next_event_debug_data_entry *entry = &data->entry[index];
	...
	entry->alarm = alarm;
	entry->now = now;
	entry->timer_count = timer_count;
	entry->timer_match = timer_match;
	entry->timer_enable = timer_enable;
				    ...
				    struct clock_event_device *evt)
{
	...
	uint32_t alarm;
	int late;
	uint32_t timer_count, timer_match, timer_enable;
	...
	timer_count = readl(clock->regbase + TIMER_COUNT_VAL);
	timer_match = readl(clock->regbase + TIMER_MATCH_VAL);
	timer_enable = readl(clock->regbase + TIMER_ENABLE);

	save_next_event_debug(alarm, now, timer_count, timer_match, timer_enable);
	...
	uint32_t src_clk_val, uint32_t src_clk_freq)
{
	...
	if (dst_clk_state->sleep_offset + dst_clk_state->non_sleep_offset !=
		...
		save_sync_debug(dst_clk, src_clk_val, dst_clk_val,
			dst_clk_state->sleep_offset, dst_clk_state->non_sleep_offset,
			data->exit_sleep);
	}
}
