...
static int mi2s_gpios_request(void)
{
	...
	if (rc < 0) {
		pr_aud_err("%s: GPIO request for MI2S_WS failed\n", __func__);
		return rc;
	}

	rc = gpio_request(snddev_mi2s_drv.mi2s_sclk, "MI2S_SCLK");
	if (rc < 0) {
		pr_aud_err("%s: GPIO request for MI2S_SCLK failed\n", __func__);
	...
	if (rc < 0) {
		pr_aud_err("%s: GPIO request for MI2S_MCLK failed\n",
			__func__);
	...
	if (rc < 0) {
		pr_aud_err("%s: GPIO request for FM_MI2S_SD failed\n",
			__func__);
		gpio_free(snddev_mi2s_drv.mi2s_ws);
		gpio_free(snddev_mi2s_drv.mi2s_sclk);
...
static int mi2s_get_gpios(struct platform_device *pdev)
{
	...
	if (!res) {
		pr_aud_err("%s: failed to get gpio MI2S_WS\n", __func__);
		return -ENODEV;
	}
	...
	if (!res) {
		pr_aud_err("%s: failed to get gpio MI2S_SCLK\n", __func__);
		return -ENODEV;
	}
	...
	res = platform_get_resource_byname(pdev, IORESOURCE_IO,
					   "mi2s_mclk");
	if (!res) {
		pr_aud_err("%s: failed to get gpio MI2S_MCLK\n", __func__);
		return -ENODEV;
	}
	...
	res = platform_get_resource_byname(pdev, IORESOURCE_IO,
					   "fm_mi2s_sd");
	if (!res) {
		pr_aud_err("%s: failed to get gpio FM_MI2S_SD\n", __func__);
		return -ENODEV;
	}
...
static int snddev_mi2s_open(struct msm_snddev_info *dev_info)
{
	...
	if (IS_ERR_VALUE(rc)) {
		pr_aud_err("ERROR setting osr clock\n");
		return -ENODEV;
	}
	...
	if (IS_ERR_VALUE(rc)) {
		pr_aud_err("ERROR setting bit clock\n");
	...
	if (snddev_mi2s_data->channel_mode == 1)
		...
		pr_aud_err("ERROR: Invalid MI2S channel mode\n");
	...
	case 1:
		...
		case MI2S_SD3:
			...
			break;
		default:
			pr_aud_err("%s: invalid SD line\n",
			__func__);
			goto error_invalid_data;
		}
		if (channels != AFE_MI2S_STEREO &&
		channels != AFE_MI2S_MONO) {
			pr_aud_err("%s: for one SD line, channel "
			"must be 1 or 2\n", __func__);
			goto error_invalid_data;
		}
	...
	case 2:
		...
		if (channels != AFE_MI2S_4CHANNELS) {
			pr_aud_err("%s: for two SD lines, channel "
	...
	case 3:
		...
		case MI2S_SD0 | MI2S_SD1 | MI2S_SD2:
			...
			break;
		default:
			pr_aud_err("%s: invalid SD lines\n",
			__func__);
			goto error_invalid_data;
		}
		if (channels != AFE_MI2S_6CHANNELS) {
			pr_aud_err("%s: for three SD lines, lines "
			"must be 1, 2, and 3\n", __func__);
			goto error_invalid_data;
		}
		break;
	case 4:
		...
		if (channels != AFE_MI2S_8CHANNELS) {
			pr_aud_err("%s: for four SD lines, lines "
			"must be 1, 2, 3, and 4\n", __func__);
			goto error_invalid_data;
		}
		break;
...
static int __init snddev_mi2s_init(void)
{
	...
	if (IS_ERR_VALUE(rc)) {
		pr_aud_err("%s: platform_driver_register for mi2s_fm_driver failed\n",
				__func__);
	...
	pr_aud_info("snddev_mi2s_init : done\n");
