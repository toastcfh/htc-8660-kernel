...
static void holiday_panel_power(int on)
{
	...
	if (!init) {
		...
		//gpio_direction_output(HOLIDAY_GPIO_LCM_RST_N, 0);
		init = 1;
	}
...
static int holiday_esd_fixup(uint32_t data)
{
   ...
   if(++check_count == ESDLOOPCOUNT) {
           check_count = 0;
        if(0x9C != (pwrVal = mipi_orise_read_power())) {
			printk("%s: power status of driver IC = 0x%02X\n",
				__func__, pwrVal);
                   hr_msleep(1);
                   if(0x9C != (pwrVal = mipi_orise_read_power())) {
				printk("%s: power status of driver IC = 0x%02X\n",
					__func__, pwrVal);
				printk("-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+\n");
                           printk("%s: driver IC failure deteted, could be ESD\n", __func__);
				printk("-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+\n");
