...
static uint32_t msm_aic3254_reset_gpio[] = {
	GPIO_CFG(SHOOTER_U_AUD_CODEC_RST, 0,
...
void shooter_u_snddev_hsed_pamp_on(int en)
{
	...
	if (en) {
		/* enable rx route */
		msleep(30);
		gpio_set_value(PM8058_GPIO_PM_TO_SYS(SHOOTER_U_AUD_HANDSET_ENO), 1);
		set_headset_amp(1);
		...
		/* disable rx route */
		set_headset_amp(0);
		gpio_set_value(PM8058_GPIO_PM_TO_SYS(SHOOTER_U_AUD_HANDSET_ENO), 0);
		if (!atomic_read(&aic3254_ctl))
			curr_rx_mode &= ~BIT_HEADSET;
	}
}
