...
/* Return number of votes for clock source without taking lock*/
static int local_src_get_votes_nolock(int src)
{
	return src_votes[src];
}

/* Return number of votes for clock source */
int local_src_get_votes(int src)
{
	int rc = 0;
	unsigned long flags;
	...
	spin_lock_irqsave(&src_vote_lock, flags);
	rc = local_src_get_votes_nolock(src);
	spin_unlock_irqrestore(&src_vote_lock, flags);
...
int local_clk_src_xo(unsigned id)
{
	int rc = 0;
	struct clk_local *clk;

	if (id >= L_NR_CLKS || soc_clk_check_ignore(id))
		goto out;

	clk = &soc_clk_local_tbl[id];
	if (clk->type == RESET)
		goto out;

	if (clk->count) {
		if (clk->parent != C(NONE)) {
			rc = local_clk_src_xo(clk->parent);
			if (rc)
				goto out;
		}
		if (clk->current_freq->src != SRC_NONE)
			rc = 1;
	}
...
int local_clk_vdig_level(unsigned id)
{
	if (id >= L_NR_CLKS)
		return 0;
	else {
		struct clk_local *clk =  &soc_clk_local_tbl[id];
		return clk->current_freq->sys_vdd;
	}
}
